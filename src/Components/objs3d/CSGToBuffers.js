...
// From: http://3d.hrg.hr/jscad/three/CSGToBuffers.js
  ...
  const extract = (list) => {
    list.forEach((csg) => {
      if (csg instanceof Array) {
        ...
        let obj = CSGToBuffers(csg, transferable, map.unique)
...
function CSGToBuffers(csg, transferable, unique) {
  let obj
  ...
  if (csg.vertices) {
    // coer a case where the object already has the format
    obj = csg
